...
for i = 1:5500                                          %generating s values
    ...
    s((((i-1)*(T*fs))+1):(i*(T*fs))) = (x(i))*(sin(2*pi*fc*n((((i-1)*(T*fs))+1):(i*(T*fs)))*Ts));
end
...
s1 = -3*sin(2*pi*fc*Ts*(0:(length(s)-1)));
s2 = -1*sin(2*pi*fc*Ts*(0:(length(s)-1)));
s3 = 1*sin(2*pi*fc*Ts*(0:(length(s)-1)));
s4 = 3*sin(2*pi*fc*Ts*(0:(length(s)-1)));
