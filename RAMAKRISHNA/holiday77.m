...
for i = 1:5500                                          %generating s values
    ...
    s((((i-1)*(T*fs))+1):(i*(T*fs))) = (x(i))*(cos(2*pi*fc*n((((i-1)*(T*fs))+1):(i*(T*fs)))*Ts));
end
...
s1 = -3*cos(2*pi*fc*Ts*(0:(length(s)-1)));
s2 = -1*cos(2*pi*fc*Ts*(0:(length(s)-1)));
s3 = 1*cos(2*pi*fc*Ts*(0:(length(s)-1)));
s4 = 3*cos(2*pi*fc*Ts*(0:(length(s)-1)));
...
approxBER = qfunc(sqrt((4*eb)/(5*No)))          %verifying with the Q function
